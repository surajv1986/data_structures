/* A simple program to demonstrate reversing a singly linked list */
...
int reverse_list(void)
{
	struct node *curr, *prev, *successor;
	int count = 1;
	...
	if (prev == NULL) {	
		...
		while (curr != NULL) {
			...
			if ((successor == NULL) || (curr->next == NULL)) {
				curr->next = prev;
				/* Adjust head for reversed list*/
				head = curr;
				break;
			}
			/* now proceed ahead on the list */
			prev = curr;
			curr = successor;
